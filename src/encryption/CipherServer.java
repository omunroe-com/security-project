...
public class CipherServer
{
        ...
        {
            //listening to the client on the same port (here, 7999)
            int port = 7999;
            ServerSocket server = new ServerSocket(port);
            Socket socket = server.accept();

            //reading the key from the file that was encrypted by the client
            ObjectInputStream in = new ObjectInputStream(new FileInputStream("OutKeyDES.txt"));
            ...
            FileOutputStream input_1 = new FileOutputStream("Output.txt");
